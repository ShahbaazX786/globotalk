...
import { getSentReqIds } from "../utils/helpers";
...
const HomePage = () => {
  ...
  const { recommendedList, isLoading: isRecommendedListLoading } =
    useGetRecommendedUserQuery();
  const sentReqIds = getSentReqIds(sentReqs, authUser._id);
        ...
        <section id="recommended-users">
          ...
          {isRecommendedListLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {recommendedList.map((user: User) => {
                const hasSentReq = sentReqIds.has(user._id);
                return (
                  <FriendCard
                    key={user._id}
                    friend={user}
